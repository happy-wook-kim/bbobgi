...
| 구역 배치 | **원형 아레나 + 부채꼴 n등분** — 인원수와 무관하게 전 구역이 같은 크기·같은 모양(합동). 벽은 원형 반사(법선 반전·감쇠) |
| 카운트다운 강타 | 마지막 5회는 `slamPower` 1.2→2.1 램프로 점점 세게 쳐서 확확 튕기고, 마지막 강타가 가장 세서 결과를 끝까지 알 수 없다 |
| 입력 | 던지기 버튼 하나 — 이후는 전부 자동 진행 |
| 통합 | 게임 레지스트리(`src/games.ts`)에 등록 — 메뉴·타입·랜덤/점수 풀 자동 반영 (2026-07-16 편입 완료). 랜덤 아이콘 🎲→🔀 변경 |
